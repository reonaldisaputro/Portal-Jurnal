    ...
    <section class="max-w-[1130px] md:flex justify-between gap-5 my-10 md:mx-auto mx-3">
        <div class="bg-white p-5">
            ...
            <div class="grid md:grid-cols-5 gap-2">
                @forelse ($author->news->unique('category_id') as $news)
                    <button
                        class="border rounded-md shadow-lg py-3 px-3 text-[#FF6B18] hover:shadow-2xl">{{ $news->category->name }}</button>
                @empty
        ...
        <div class="bg-white p-5">
            ...
            @if (!empty($author->linkedin))
                <a href="{{ $author->linkedin }}" class="hover:underline py-3">
            ...
            @if (!empty($author->instagram))
                <a href="{{ $author->instagram }}" class="hover:underline py-3">
                    <p class="font-bold">Instagram</p>
                    <p>{{ $author->instagram }}</p> <!-- Username diambil dari field Instagram -->
                </a>
            @endif
            <!-- Jika ada link ke Instagram -->
            @if (!empty($author->youtube))
                <a href="{{ $author->youtube }}" class="hover:underline py-3">
                    <p class="font-bold">Youtube</p>
                    <p>{{ $author->youtube }}</p> <!-- Username diambil dari field Instagram -->
                </a>
            @endif
            @if (!empty($author->tiktok))
                <a href="{{ $author->tiktok }}" class="hover:underline py-3">
                    <p class="font-bold">Tiktok</p>
                    <p>{{ $author->tiktok }}</p> <!-- Username diambil dari field Instagram -->
                </a>
            @endif

            <!-- Jika ada link ke Twitter -->
            @if (!empty($author->twitter))
                <a href="{{ $author->twitter }}" class="hover:underline">
                    <p class="font-bold">Twitter</p>
                    <p>{{ $author->twitter }}</p> <!-- Username diambil dari field Twitter -->
                </a>
            @endif
